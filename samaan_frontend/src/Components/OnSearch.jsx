...
const OnSearch = () => {
  ...
  const fetchdata = async () => {
    await fetch(`https://api-krudra9125-gmailcom.vercel.app/api/searchbarRequestedData/${id}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  useEffect(() => {
    if (brandDataFilter!==null) {
      setfilteredData(product_data.filter((item) => item.brand === brandDataFilter));
    }
  }, [brandDataFilter]);
        ...
        <div className="w-[100%] h-[100%] max-w-screen-2xl mx-auto flex flex-col justify-start items-center min-h-fit  ">
          ...
          <div className="OnSearchOuter Vflex AroundFlex">
            <div className="OnSearchInnerLeft">
              <OnSearchLeft data={brandsdata} filterFunction={setbrandDataFilter} />
            </div>
